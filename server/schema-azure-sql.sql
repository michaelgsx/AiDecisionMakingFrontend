...
--------------------------------------------------------------------------------
-- 4. Risk embeddings — vector embeddings per request
--    A single request may have multiple embedding types (feature, conversation,
--    graph, etc.) and may be re-embedded with different models over time.
--------------------------------------------------------------------------------
...
BEGIN
  CREATE TABLE dbo.risk_embeddings (
    id              BIGINT        IDENTITY(1, 1) NOT NULL PRIMARY KEY,
    request_id      CHAR(36)      NOT NULL,
    embedding_type  VARCHAR(30)   NOT NULL,     -- 'feature' | 'conversation' | 'graph'
    embedding_json  NVARCHAR(MAX) NOT NULL,     -- vector as JSON array of floats
    dimensions      INT           NOT NULL,     -- vector length for quick validation
    model_name      NVARCHAR(200) NOT NULL,     -- e.g. 'text-embedding-3-small'
    model_version   NVARCHAR(50)  NULL,         -- e.g. '2024-02-01', nullable for unversioned models
    created_at      DATETIME2     NOT NULL
      CONSTRAINT DF_risk_embeddings_created_at DEFAULT SYSUTCDATETIME(),
    CONSTRAINT CK_risk_embeddings_type
      CHECK (embedding_type IN ('feature', 'conversation', 'graph')),
    CONSTRAINT FK_risk_embeddings_request
      FOREIGN KEY (request_id) REFERENCES dbo.risk_features (request_id),
    CONSTRAINT UK_risk_embeddings_request_type_model
